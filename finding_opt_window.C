...
//=============================================================
tuple<double,double,double,double> Significance_Optimization(double Lumi_scalefactor, bool plot){
//=============================================================
...
if(plot==1){
  ...
  if(fabs(Lumi_scalefactor-1.00)<0.01){
	...
	optWinSig->SetLineColor(kBlack);
	optWinSig->SetLineStyle(2);
	optWinSig->Draw("same");

	TLine* optSigSig = new TLine(0,largest_exp_sig, 25,largest_exp_sig);
	optSigSig->SetLineWidth(2);
	optSigSig->SetLineColor(kBlack);
	optSigSig->SetLineStyle(2);
	optSigSig->Draw("same");

  //-- axes
  AddText( 0.900, 0.035, "Mass window[GeV]",0.060, 0.,"right"); // X-axis
  AddText( 0.33, 0.950, "Significance" ,0.060,0.,"right");   // Y-axis
  AddText( 0.45, 0.825, Form("Integrated luminosity scalefactor = %5.1f",Lumi_scalefactor),0.0450, 0.,"left");
